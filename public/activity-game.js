// PSEUDO: load Discord Activity SDK (actual URL depends on Discord docs)
// <script src="https://discord.com/activity-sdk.js"></script> in HTML normally
...
// Activity SDK init (pseudo)
window.DiscordActivity?.init().then(activity => {
  activity.getUser().then(user => {
    discordUser = user;
    startGame();
  });
}).catch(err => {
...
function connectServer() {
  ...
  ws.onopen = () => {
    const name = discordUser ? discordUser.username : `Guest${Math.floor(Math.random()*999)}`;
    const discordId = discordUser ? discordUser.id : 'guest';
  ...
  ws.onmessage = (event) => {
    ...
    if (data.type === 'init') {
      ...
      // check achievements client-side for UI
